...
def n2vDenoise(model_dir, model_name, image_dir, image_name, channel_name, plotInputPredict=False):
	'''
	...
	model = N2V(config=None, name=channel_name, basedir=model_dir)
	# We load the data we want to process.
	split_image_dir=os.path.join(image_dir, 'n2v/TMP/' + channel_name)
	...
	if(plotInputPredict):
		inputpredict_path=os.path.join(image_dir,'n2v/inputPredict/')
		if not os.path.isdir(inputpredict_path):
			os.makedirs(inputpredict_path, exist_ok=True)
		...
		plt.savefig(os.path.join(inputpredict_path, "InputPredict_" +re.sub("\.nd2", "_" + channel_name + ".tif", image_name)))
	
	# output denoised images
	denoised_path=os.path.join(image_dir,'n2v/denoised/')
...
def splitChannels(image_dir, image_name, channel_list):
	...
	if (len(img.metadata['channels'])>1):
	# if(len(img.shape)>3):
		print("splitting channels")
		img.bundle_axes = 'zyx'
		img.iter_axes = 'c'
		for i in range(len(channel_list)): 
			tempChannel_path=os.path.join(image_dir, 'n2v/TMP/' + channel_list[i])
		...
		tempChannel_path=os.path.join(image_dir, 'n2v/TMP/' + channel_list[0])
		if not os.path.isdir(tempChannel_path):
			os.makedirs(tempChannel_path, exist_ok=True)
...
def main():
	...
	for channel_name in channel_list:
		...
		n2vDenoise(model_dir, model_name, image_dir, image_name, channel_name, plotInputPredict=True)
		split_image_dir=os.path.join(image_dir, 'n2v/TMP/' + channel_name)
		split_image_name=re.sub("\.nd2", "_" + channel_name + ".tif", image_name)
